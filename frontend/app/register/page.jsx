...
const RegisterPage = () => {
  ...
  return (
    <div className="bg-[#0B0F19] min-h-screen flex items-center justify-center px-4 sm:px-6 py-8 relative overflow-hidden">
      ...
      <div className="w-full max-w-md bg-[#111625] rounded-2xl sm:rounded-3xl border border-white/[0.04] p-5 sm:p-8 md:p-10 shadow-2xl space-y-6 relative z-10">
        ...
        <div className="space-y-2">
          ...
          <p className="text-xs sm:text-sm text-slate-400 font-light leading-relaxed">
            Join Manoj Cart to claim exclusive product drops and fast allocations.
          </p>
        </div>

        {/* Success Banner Overlay Panel */}
        {successMsg && (
          <div className="p-4 sm:p-5 text-xs text-emerald-400 bg-emerald-500/10 rounded-xl border border-emerald-500/20 font-medium tracking-wide space-y-4 leading-relaxed">
            <p className="flex items-center gap-2">✨ {successMsg}</p>
            <div>
              <Link
                href="/login"
                className="inline-flex items-center justify-center w-full sm:w-auto bg-gradient-to-r from-emerald-500 to-teal-500 hover:from-emerald-400 hover:to-teal-400 text-slate-950 font-black tracking-wider uppercase text-[10px] px-4 py-2.5 rounded-xl shadow-lg shadow-emerald-500/10 transition-transform duration-200 hover:scale-[1.02]"
              >
                Go to Login Page →
              </Link>
            </div>
          ...
          <form onSubmit={handleSubmit} method="POST" className="space-y-5">
            <div className="space-y-1.5">
              ...
              <input
                ...
                className="w-full px-4 py-3 sm:py-3.5 bg-slate-950/40 text-slate-100 placeholder-slate-600 border border-white/[0.04] focus:border-emerald-500/50 rounded-xl text-sm transition-all duration-300 outline-none shadow-inner disabled:opacity-40"
        ...
        <div className="pt-2 text-center text-xs sm:text-sm text-slate-500 font-medium tracking-wide">
