...
const ProductPage = () => {
  ...
  return (
    <div ref={pageContainerRef} className="bg-[#0B0F19] min-h-screen text-white p-6 sm:p-8 md:p-12 space-y-8">
      
      {/* Structural Header Section */}
      <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-6 border-b border-white/[0.04] pb-6">
        <div>
      ...
      {loading ? (
        ...
        <div className="text-center py-24 text-slate-500 bg-[#111625]/20 rounded-3xl border border-white/[0.04] border-dashed max-w-2xl mx-auto space-y-2">
          <p className="text-lg font-bold text-slate-400">No Premium Products Found</p>
          <p className="text-sm font-light text-slate-500">Try adjusting your keywords or browse another category drop.</p>
        </div>
      ) : (
        <>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-8">
            {products.map((product) => (
              <div key={product.id || product._id} className="catalog-product-wrapper opacity-0">
          ...
          <div className="flex justify-center items-center pt-12 gap-3 select-none">
            <button
              onClick={() => setPage((p) => Math.max(p - 1, 1))}
              disabled={page === 1}
              className="px-5 py-2.5 rounded-xl text-xs uppercase tracking-wider font-black bg-white/[0.02] border border-white/[0.04] text-slate-300 hover:bg-emerald-500 hover:text-slate-950 hover:border-transparent disabled:opacity-30 disabled:hover:bg-white/[0.02] disabled:hover:text-slate-300 disabled:hover:border-white/[0.04] transition-all duration-300 cursor-pointer disabled:cursor-not-allowed"
            >
              Previous
            </button>

            <span className="px-5 py-2.5 rounded-xl text-xs font-bold tracking-wide bg-[#111625] border border-white/[0.04] text-slate-400">
              Page <span className="text-emerald-400 font-black">{page}</span> of {totalPages}
            </span>
