...
const PremiumHero = () => {
  ...
  return (
    <section
      ...
      className="relative min-h-[92vh] flex items-center py-20 bg-[#0B0F19] text-white overflow-hidden transition-all duration-300
                 before:content-[''] before:absolute before:inset-0 before:opacity-[0.15] before:pointer-events-none
                 before:bg-[radial-gradient(circle_800px_at_var(--mouse-x,0px)_var(--mouse-y,0px),#10B981_0%,transparent_100%)]"
    >
      ...
      <div className="container mx-auto px-6 grid grid-cols-1 lg:grid-cols-2 gap-16 items-center relative z-10">
        ...
        <div className="flex flex-col items-start space-y-6">
          ...
          <h1 className="anim-title text-[44px] sm:text-5xl lg:text-[64px] font-black tracking-tight leading-[1.05]">
          ...
          <p className="anim-subtitle text-slate-400 text-base lg:text-lg max-w-lg leading-relaxed font-light">
          ...
          <div className="anim-buttons flex flex-wrap gap-4 w-full sm:w-auto pt-2">
            <Link
              href="/product"
              className="group bg-gradient-to-r from-emerald-500 to-teal-500 hover:from-emerald-400 hover:to-teal-400 text-slate-950 font-black tracking-wide text-sm px-8 py-4 rounded-xl shadow-xl shadow-emerald-500/20 transition-all duration-300 hover:scale-[1.02] hover:shadow-emerald-400/30 active:scale-[0.98]"
            >
            ...
            <Link
              href="/product"
              className="backdrop-blur-md bg-white/5 border border-white/10 text-white font-semibold text-sm px-8 py-4 rounded-xl transition-all duration-300 hover:bg-white/10 hover:border-white/20 hover:scale-[1.02] active:scale-[0.98]"
            >
              View Lookbook
            </Link>
          </div>
          ...
          <div className="anim-trust flex gap-8 pt-8 border-t border-white/5 w-full max-w-sm text-xs text-slate-500 tracking-wider uppercase font-medium">
        ...
        <div
          ref={visualStageRef}
          className="relative flex justify-center lg:justify-end items-center w-full min-h-[440px] [perspective:1200px]"
        >
          {/* Inner 3D Transformer Canvas */}
          <div className="graphic-perspective-box relative flex gap-3 sm:gap-4 items-center p-12 bg-white/[0.02] border border-white/5 rounded-3xl [transform-style:preserve-3d] shadow-2xl backdrop-blur-3xl">
            
            {/* Absolute Rotating Accent Sticker */}
            <div className="anim-sticker absolute -top-6 -left-6 z-30 bg-gradient-to-br from-amber-400 to-orange-500 text-slate-950 font-black text-xs px-4 py-2.5 rounded-2xl shadow-xl shadow-orange-500/20 tracking-wider uppercase select-none [transform:translateZ(50px)]">
              🔥 SAVE UP TO 50%
            </div>
            ...
            {[
              ...
              <div
                key={idx}
                className={`interactive-tag ${item.tagClass} w-[72px] h-[175px] sm:w-[85px] sm:h-[200px] lg:w-[95px] lg:h-[230px] bg-gradient-to-b ${item.grad} rounded-b-3xl rounded-t-[45px] flex flex-col items-center justify-end pb-8 relative shadow-2xl shadow-black/40 [transform-style:preserve-3d]`}
              >
                ...
                <span className="text-white text-4xl sm:text-5xl lg:text-6xl font-black tracking-tight select-none font-serif [transform:translateZ(30px)] drop-shadow-[0_4px_12px_rgba(0,0,0,0.3)]">
                  {item.char}
                </span>
              </div>
            ))}
