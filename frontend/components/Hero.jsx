...
const Hero = () => {
  ...
  return (
    <section
      ref={heroRef}
      className="min-h-screen bg-gradient-to-br from-emerald-50/40 via-white to-teal-50/30 flex items-center py-12 sm:py-16 lg:py-24 overflow-hidden"
    >
      ...
      >
        ...
        <div className="flex flex-col items-center lg:items-start text-center lg:text-left z-10">
          ...
          <h1 className="hero-title text-4xl sm:text-5xl md:text-6xl lg:text-[62px] font-black text-slate-900 leading-[1.1] tracking-tight">
            Elevate Style & <br />
          ...
          <div className="hero-buttons flex flex-col sm:flex-row gap-4 mt-8 w-full sm:w-auto">
            <Link
              href="/product"
              className="w-full sm:w-auto bg-emerald-600 hover:bg-emerald-700 text-white text-center px-8 py-3.5 rounded-xl font-bold text-sm transition-all"
            >
              Shop Collection
            </Link>
            ...
            <Link
              href="/product"
              className="w-full sm:w-auto border border-slate-200 bg-white text-slate-700 text-center px-8 py-3.5 rounded-xl font-semibold text-sm transition-all"
            >
              Explore Products
            </Link>
          </div>
          ...
          <div
            className="hero-stats flex justify-center lg:justify-start gap-8 sm:gap-12 mt-12 pt-8 border-t border-slate-100 w-full max-w-md"
          >
            <div>
        ...
        <div
          ...
          className="relative flex justify-center lg:justify-end items-center w-full min-h-[300px] sm:min-h-[400px] [perspective:1000px]"
        >
          {/* Inner 3D perspective orientation node box wrapper */}
          <div className="hero-image-container relative flex gap-2 sm:gap-3 lg:gap-4 items-center p-4 sm:p-8 [transform-style:preserve-3d]">
            {/* Absolute Floating Sticker Badge */}
            <div
              className="discount-badge absolute -top-2 sm:-top-4 right-0 z-30 bg-teal-500 text-white px-3 sm:px-4 py-1.5 sm:py-2 rounded-full font-black text-[10px] sm:text-xs shadow-xl flex items-center gap-1"
            >
              🔥 Up To 50% OFF
            </div>
            ...
            {['S', 'A', 'L', 'E'].map((letter, index) => (
              <div
                key={index}
             className="hero-tag-item w-[55px] h-[135px] sm:w-[75px] sm:h-[180px] lg:w-[95px] lg:h-[220px] bg-gradient-to-b from-emerald-500 to-emerald-600 rounded-b-2xl rounded-t-[40px] flex flex-col items-center justify-end pb-6 lg:pb-8 relative shadow-xl"
              >
                ...
                <span className="text-white text-3xl sm:text-5xl lg:text-6xl font-black tracking-tighter select-none font-serif [transform:translateZ(25px)] drop-shadow-md">
                  {letter}
                </span>
              </div>
            ))}
